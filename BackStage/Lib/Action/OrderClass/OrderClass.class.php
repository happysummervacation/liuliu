<?php
	class OrderClass extends Action{
		...
		*/
		...
		/*
		*俞鹏泽
		*获取本月的学生订课情况(这里的创建月评情况是根据一对一课表来进行创建)
		*/
		public static function GetTeacherAndStudentMonthSituationService(){
			import("BackStage.Action.DBConfig.Config");
			import("BackStage.Action.Public.PublicFunction");

			$month_start_time = PublicFunction::GetThisMonthStartEndTime()[0];
			$month_end_time = PublicFunction::GetThisMonthStartEndTime()[1];
			...
			$sql = "
			 ...
			 inner join tp_class on tp_class.classID=tp_oneorderclass.classID and
			 tp_oneorderclass.isdelete=0 and (classStatus!=2 and classStatus!=6)
			 and tp_class.classStartTime>={$month_start_time} and
			 tp_class.classStartTime<={$month_end_time}) as tem
			 group by tem.studentID,tem.teacherID
			 order by number desc
			 ";
			 //由于是降序排序,所以第一个值就是最大的值
