...
<body>
                        ...
                        <div class="panel-body" style="overflow: auto;">
                            <div class="dataTable_wrapper" style="max-width: 100%;">
                              <table class="table table-striped table-bordered table-hover" id="dataTables-example2" style="max-width: 100%;">
                                ...
                                <tbody>
                                  <?php if(is_array($groupClassInfo)): foreach($groupClassInfo as $key=>$vo): ?><tr>
                                        <td value='<?php echo ($vo["groupClassSchID"]); ?>'><?php echo ($vo["groupClassSchID"]); ?></td>
                                        <td><?php echo (date('Y-m-d H:i',$vo["classStartTime"])); ?>-
                                            <?php echo (date('H:i',$vo["classEndTime"])); ?></td>
                                        <td>
                                            <?php switch($vo['gclassStatus']): case "0": ?>还没有上课<?php break;?>
                                                <?php case "1": ?>正常上课<?php break;?>
                                                <?php case "2": ?>学生退课<?php break;?>
                                                <?php case "3": ?>教师早退<?php break;?>
                                                <?php case "4": ?>教师迟到<?php break;?>
                                                <?php case "5": ?>教师缺课<?php break;?>
                                                <?php case "6": ?>教师退课<?php break;?>
                                                <?php case "7": ?>意外情况<?php break; endswitch;?>
                                        </td>
                                        <td>
                                            <?php if(empty($vo['comment'])){ echo "还没有进行评论"; }else{ echo $vo['comment'][0]['commentlevel']; } ?>
                                        </td>
                                        <td>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form class="form" action="<?php echo U('GroupClass/GroupClassManage',array('type'=>'changeStatus'));?>" method="post">
    ...
    <script type="text/javascript">
      ...
      $('.submit').click(function(){
        ...
        $.ajax({
          url:"<?php echo U('GroupClass/GroupClassManage',array('type'=>'addGroupClass','groupID'=>$groupID));?>",
          type:'post',
          data:'times='+data,
      ...
      $('.classnote').click(function(){
        $.ajax({
          url:'<?php echo U('GroupClass/GroupClassManage',array('type'=>'getNote'));?>',
          type:'post',
      ...
      $('.teacherreturnclass').click(function(){
        ...
        if(confirm('教师退课将不消耗学生课时！\n请确认您的操作！')){
          window.location.href='<?php echo U('GroupClass/GroupClassManage',array('type'=>'teaCancelClass'));?>/groupClassSchID/'+data//记得加上data数据
        }else{
          return;
        }
      });
